...
- **context** — Represents the context for a worker process `parsing-subprocess-pool.ts:242-242`
- **context** — Handles worker messages and unexpected exits `parsing-subprocess-pool.ts:243-243`
- **context** — onUnexpectedExit is a function that handles worker exit events `parsing-subprocess-pool.ts:244-244`
...
- **filteredEntities** — Filters entities for embedding generation `embedding-processor.ts:421-421`
- **filteredEntities** — Filters entities to exclude those of excluded types `embedding-processor.ts:633-633`
...
- **GENERATED_FILENAME_MARKERS** — Represents markers for generated filenames `generic-language-worker.ts:267-267`
- **GENERATED_FILENAME_MARKERS** — Checks if the filename ends with a minified file extension `generic-language-worker.ts:268-268`
- **GENERATED_HEADER_MARKERS** — Placeholder for generated header markers `generic-language-worker.ts:238-238`
- **GENERATED_HEADER_MARKERS** — Checks if the header includes specific generated code markers `generic-language-worker.ts:242-242`
- **GENERATED_HEADER_MARKERS** — Checks if the header includes "code generated by protoc-gen" and sets type to "gRPC" `generic-language-worker.ts:244-244`
- **GENERATED_HEADER_MARKERS** — Checks if the header includes "autogenerated by thrift" and sets type to "Thrift" `generic-language-worker.ts:246-246`
- **GENERATED_HEADER_MARKERS** — Checks if the header includes "a bison parser" or "generated by bison" and sets type to "Bison" `generic-language-worker.ts:248-248`
- **GENERATED_HEADER_MARKERS** — Checks if the header includes "generated by flex" or "a lexical scanner generated by flex" and sets type to "Flex" `generic-language-worker.ts:251-251`
- **GENERATED_HEADER_MARKERS** — Checks if the header includes "swig" and "generated" and sets type to "SWIG" `generic-language-worker.ts:255-255`
- **GENERATED_HEADER_MARKERS** — Checks if the header includes "generated by peg" or "generated by peggy" and sets type to "PEG". `generic-language `generic-language-worker.ts:257-257`
- **GENERATED_HEADER_MARKERS** — Checks if a header includes "tree-sitter" and "generated" `generic-language-worker.ts:259-259`
...
- **promises** — Manages promises for asynchronous operations `parsing-subprocess-pool.ts:1044-1051`
- **promises** — Submits a single task and returns the result with timing information `parsing-subprocess-pool.ts:1047-1050`
- **pythonFiles** — Manages Python-specific files and their parsing `python-worker-pool.ts:224-224`
- **readyHandler** — Handles the "ready" event from worker threads `language-worker-pool.ts:341-351`
- **readyHandler** — Handles the "ready" message from a worker, sets the resolved flag, updates the worker state, and resolves the ready promise `language-worker-pool.ts:355-362`
...
- **createWorker** — Creates a new worker thread for the pool `python-worker-pool.ts:150-217`
- **createWorker** — Initializes a new worker thread for the pool `worker-pool-manager.ts:108-169`
...
- **submitTask** — Submits a task to the worker pool for processing `python-worker-pool.ts:222-265`
- **submitTask** — Adds a new task to the worker pool for processing `worker-pool-manager.ts:174-205`
...
- **avgTimePerFile** — Average time taken per file in the task `parser-worker.ts:29-29`
- **avgTimePerFile** — Average time taken per file processed `python-worker.ts:33-33`
...
- **completedTasks** — Number of tasks completed `language-worker-pool.ts:115-115`
- **completedTasks** — Counts the number of completed tasks `language-worker-pool.ts:153-153`
- **completedTasks** — Represents the set of tasks that have been successfully completed by the subprocess pool `parsing-subprocess-pool.ts:124-124`
- **completedTasks** — Keeps track of tasks that have been successfully completed `python-worker-pool.ts:93-93`
- **completedTasks** — Private variable to count completed tasks `python-worker-pool.ts:110-110`
- **completedTasks** — The number of completed tasks `worker-pool-manager.ts:54-54`
- **completedTasks** — Tracks the number of completed tasks `worker-pool-manager.ts:67-67`
...
- **count** — Embeddings ready response (binary embeddings from worker) `parsing-subprocess-pool.ts:70-70`
- **count** — Optional count property `parsing-subprocess-pool.ts:85-85`
...
- **description** — Description of the entity `embedding-processor.ts:26-26`
- **description** — Parameters for the function, including name, type, and description `embedding-processor.ts:27-27`
- **description** — Returns a type and description for the function `embedding-processor.ts:28-28`
...
- **embedding** — Contains the embedding result from the API `worker-embedding-client.ts:42-42`, `worker-embedding-client.ts:318-318`
- **embedding** — OVMS/OpenAI API embedding response `worker-embedding-client.ts:27-27`
...
- **failedTasks** — Logs tasks that failed during processing `python-worker-pool.ts:94-94`
- **failedTasks** — Private variable to count failed tasks `python-worker-pool.ts:111-111`
- **failedTasks** — The number of failed tasks `worker-pool-manager.ts:55-55`
- **failedTasks** — Tracks the number of failed tasks `worker-pool-manager.ts:68-68`
...
- **file** — Represents a single file being parsed `parser-worker.ts:25-25`
- **file** — Stores an array of error messages with file and message details `parser-worker.ts:79-79`
...
- **files** — Array of file paths to be parsed `parser-worker.ts:18-18`
- **files** — Manages files related to parsing tasks `parsing-subprocess-pool.ts:1157-1157`
- **files** — Manages Python files with optimized 4-layer architecture for maximum parallelism `python-worker-pool.ts:40-40`
- **files** — Array of file paths to be processed by the Python parser `python-worker.ts:22-22`
- **files** — Represents the files to be processed `worker-pool-manager.ts:42-42`
- **filesProcessed** — Number of files processed `generic-language-worker.ts:220-220`
- **filesProcessed** — Number of files processed by a worker `language-worker-pool.ts:82-82`
- **filesProcessed** — Tracks the number of files processed `language-worker-pool.ts:118-118`
- **filesProcessed** — Number of files processed in the task `parser-worker.ts:27-27`
- **filesProcessed** — The number of files processed in the task `python-worker-pool.ts:65-65`
- **filesProcessed** — Count of files processed by the Python parser `python-worker.ts:31-31`
...
- **id** — Unique identifier for a text item `embedding-processor.ts:68-68`
- **id** — Stores the unique identifier for the entity `embedding-processor.ts:173-173`
- **id** — Stores a unique identifier for the language worker `generic-language-worker.ts:68-68`, `generic-language-worker.ts:208-208`
...
- **id** — The unique identifier for the task in the WorkerResultMessage `python-worker-pool.ts:39-39`, `python-worker-pool.ts:60-60`
- **id** — Unique identifier for a worker `python-worker-pool.ts:24-24`, `python-worker-pool.ts:79-79`
- **id** — Unique identifier for a worker task `python-worker.ts:21-21`
- **id** — Unique identifier for the worker thread `worker-pool-manager.ts:32-32`
- **id** — Represents the unique identifier for the worker pool manager `worker-pool-manager.ts:41-41`
...
- **language** — Language of the files to be parsed `generic-language-worker.ts:210-210`
- **language** — Stores the language identifier `generic-language-worker.ts:223-223`
...
- **metadata** — Additional metadata for a text item `embedding-processor.ts:70-70`
- **metadata** — Optional metadata for the entity `embedding-processor.ts:175-175`
...
- **options** — Optional parser options for the task `parser-worker.ts:19-19`
- **options** — Stores configuration options for parsing tasks `parsing-subprocess-pool.ts:1158-1158`
- **options** — Configures parsing options for Python files `python-worker-pool.ts:41-41`
- **options** — Optional parser options for the Python parser `python-worker.ts:23-23`
...
- **parseBatch** — Not directly described in the provided code `analyzer-loader.ts:416-416`
- **parseBatch** — Parses a batch of analysis results `analyzer-loader.ts:423-423`
...
- **poolSize** — Determines the number of workers in the pool `python-worker-pool.ts:115-115`
- **poolSize** — Optional size of the worker pool `python-worker-pool.ts:120-120`
- **poolSize** — The size of the worker pool `worker-pool-manager.ts:72-72`
- **poolSize** — Represents the size of the worker pool `worker-pool-manager.ts:77-77`
...
- **taskId** — The task ID associated with the results in the WorkerResultMessage `python-worker-pool.ts:61-61`
- **taskId** — Optional task ID for tracking `python-worker-pool.ts:78-78`
- **taskId** — Unique identifier for the result of a parsing task `python-worker.ts:27-27`
- **taskId** — Unique identifier for the task being processed `worker-pool-manager.ts:22-22`
- **taskId** — Stores the unique identifier for the task `worker-pool-manager.ts:24-24`
...
- **taskTimeout** — Sets the maximum time a task can be processed before it is considered failed `python-worker-pool.ts:116-116`
- **taskTimeout** — Optional timeout for tasks `python-worker-pool.ts:121-121`
...
- **text** — The actual text content `embedding-processor.ts:69-69`
- **text** — Represents a text string `embedding-processor.ts:174-174`
- **text** — Initializes an array to store entity texts and their corresponding metadata `embedding-processor.ts:424-424`
...
- **totalFilesProcessed** — Counts the total number of files processed `language-worker-pool.ts:156-156`
- **totalFilesProcessed** — Represents the total number of files processed by the parsing subprocess pool `parsing-subprocess-pool.ts:127-127`
...
- **totalTime** — Total time taken to process the task `parser-worker.ts:28-28`
- **totalTime** — The total time taken to process the task `python-worker-pool.ts:64-64`
- **totalTime** — Total time taken to process all files `python-worker.ts:32-32`
- **totalTime** — Represents the total processing time for tasks in the worker pool `worker-pool-manager.ts:26-26`
...
- **type** — Type of the entity `embedding-processor.ts:27-27`
...
- **type** — The type of the message, which is "ready" for WorkerReadyMessage `python-worker-pool.ts:51-51`
- **type** — Indicates the type of the result or error `python-worker-pool.ts:59-59`
- **type** — Indicates the type of the entity as "error" `python-worker-pool.ts:77-77`
...
- **workerScript** — Specifies the script used to create worker threads `language-worker-pool.ts:158-158`
- **workerScript** — Stores the script used for worker processes `parsing-subprocess-pool.ts:130-130`
- **workerScript** — Manages Python-specific workers for maximum parallelism on slow Python parsing `python-worker-pool.ts:114-114`
