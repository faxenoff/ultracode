...
- **allAttrs** — Creates an array of attribute names from decorators and attributes `csharp.ts:921-921`
- **allocCalls** — Filters calls to find alloc-related calls `zig.ts:18-18`
- **allocCalls** — Checks if the call name or target contains "alloc" or "create" `zig.ts:47-47`
- **allRethrow** — Returns `csharp.ts:638-638`
- **anyParams** — Not applicable in this context `typescript.ts:91-91`
- **awaitsInLoop** — Checks if the location of a loop is within the bounds of another location `typescript.ts:713-716`
- **awaitsInLoop** — Validates await usage in loops `typescript.ts:715-715`
- **bareCatchCount** — Counts bare catch blocks in exception handling `zig.ts:77-77`
- **boolParams** — Filters the entity's method parameters to find boolean parameters `csharp.ts:891-891`
- **boolParams** — Not present in the provided code snippet `python.ts:445-448`
- **boolParams** — Counts boolean parameters `typescript.ts:243-243`
- **callNames** — Not applicable in this context `java.ts:282-282`
- **callNames** — Gets the names of the calls `typescript.ts:301-301`
- **callNames** — Extracts call names from entity metadata `typescript.ts:336-336`
- **callNames** — Stores names of called functions `typescript.ts:687-687`
- **callNames** — Represents the names of calls within an entity `typescript.ts:988-988`
- **checkAccumulatingSpread** — Detects patterns indicating the use of reduce with spread in entity calls `typescript.ts:383-398`
...
- **checkAsyncConstructor** — Checks for async constructor usage `typescript.ts:275-291`
...
- **checkAwaitInLoop** — Validates await usage in loops `typescript.ts:702-726`
- **checkBareExcept** — Checks for bare except in Python code `python.ts:11-21`
- **checkBooleanBlindness** — Evaluates the entity's method parameters to detect boolean blindness `csharp.ts:888-899`
- **checkBooleanTrap** — Checks for boolean trap in the entity `typescript.ts:241-252`
- **checkBoolTrap** — Checks for boolean parameters in functions or methods `python.ts:437-456`
- **checkCaptiveDependency** — Parses the entity's metadata to check for AddSingleton and scoped service resolution `csharp.ts:771-792`
...
- **checkCircularDependency** — Checks for circular dependencies `typescript.ts:905-956`
- **checkClosureEvalScopeLeak** — Checks for eval scope leaks in closures `typescript.ts:1029-1041`
- **checkClosureFullObjectCapture** — Checks for full object capture in closures `typescript.ts:1003-1022`
- **checkClosureNoCleanup** — Checks for closures that do not clean up `typescript.ts:1108-1140`
- **checkConditionalTypeAbuse** — Identifies abuse of conditional types `typescript.ts:598-609`
...
- **checkDeclarationMergingTrap** — Checks for declaration merging traps `typescript.ts:879-899`
...
- **checkDynamicPropertyInLoop** — Validates dynamic property access in loops `typescript.ts:215-234`
...
- **checkEfSaveChangesNoTransaction** — Parses entity metadata to check for SaveChanges calls and absence of transaction usage `csharp.ts:603-623`
...
- **checkEnumPitfalls** — Checks for pitfalls in enum usage `typescript.ts:566-580`
- **checkEventHandlerLeak** — Identifies event handler leaks by counting += and -= operations in entity metadata. `csharp.ts `csharp.ts:684-730`
- **checkEventListenerLeak** — Analyzes entity metadata to detect potential memory leaks in event listener usage `typescript.ts:332-359`
- **checkExceptionFlowControl** — Checks for try-catch patterns within loops in entity metadata `csharp.ts:651-678`
- **checkExcessiveOptionalChaining** — Validates excessive optional chaining usage `typescript.ts:170-182`
- **checkGilThread** — Identifies CPU-bound operations in thread pools `python.ts:518-529`
...
- **checkImplementationTesting** — Checks for implementation testing issues `typescript.ts:634-654`
- **checkInconsistentReturn** — Validates inconsistent return types `typescript.ts:257-270`
...
- **checkJsonDeepClone** — Checks for patterns indicating deep cloning of JSON objects `typescript.ts:364-378`
...
- **checkLinqPrematureMaterialization** — Analyzes the entity's metadata to detect premature materialization in LINQ queries `csharp.ts:798-833`
...
- **checkMapInsteadOfWeakMap** — Checks for using `Map` instead of `WeakMap `typescript.ts:1052-1071`
...
- **checkMissingGenericConstraint** — Identifies missing generic constraints `typescript.ts:543-559`
- **checkMissingInputValidation** — Validates input parameters `typescript.ts:819-843`
- **checkMissingNullCheck** — Validates missing null checks in an entity `typescript.ts:758-784`
- **checkMissingReadonly** — Checks for missing readonly properties `typescript.ts:789-814`
- **checkMissingRepr** — Flags classes without a __repr__ method `python.ts:595-608`
...
- **checkNamespaceAntipattern** — Detects namespace antipatterns `typescript.ts:585-593`
...
- **checkNonNullAssertionAbuse** — Checks for abuse of non-null assertions `typescript.ts:440-451`
- **checkNoSeed** — Identifies calls to random functions without a seed `python.ts:463-478`
...
- **checkNullishVsOrConfusion** — Identifies confusion between nullish coalescing and logical OR `typescript.ts:456-467`
- **checkOpenNoEncoding** — Verifies that open() is not used without encoding `python.ts:398-410`
- **checkParameterMutation** — Validates parameter mutation in functions `typescript.ts:488-499`
...
- **checkQuadraticArrayOps** — Validates quadratic array operations `typescript.ts:296-316`
- **checkRawTypes** — Identifies raw types used without type parameters `java.ts:19-39`
- **checkRedosVulnerability** — Detects potential vulnerabilities in Redos `typescript.ts:520-538`
...
- **checkRxjsSubscribeInLoop** — Validates RxJS subscribe usage in loops `typescript.ts:683-697`
- **checkSetInsteadOfWeakSet** — Checks for using `Set` instead of `WeakSet `typescript.ts:1081-1098`
- **checkShapeDivergence** — Not applicable in this context `typescript.ts:141-165`
- **checkSideEffectImport** — Checks for side effects in imports `typescript.ts:848-873`
...
- **checkSnapshotAbuse** — Detects abuse of snapshot testing `typescript.ts:616-629`
- **checkSpreadInHotPath** — Checks for spread in hot path `typescript.ts:187-210`
...
- **checkStringlyTypedApi** — Checks for stringly typed API calls `typescript.ts:659-673`
...
- **checkTestFloatEq** — Detects float equality assertions in test files `python.ts:485-497`
- **checkThreadLocalLeak** — Detects ThreadLocal.set() without finally { remove() } `java.ts:180-216`
- **checkThreadpoolNoMax** — Checks for missing max_workers in ThreadPoolExecutor calls `python.ts:534-543`
- **checkThrowNonError** — Detects throwing non-error types `typescript.ts:472-483`
...
- **checkUnsafeIndexAccess** — Validates unsafe index access in an entity `typescript.ts:733-753`
- **checkUnsafeInnerHtml** — Checks for unsafe innerHTML usage `typescript.ts:504-515`
- **checkUnsafeOptionalUnwrap** — Not present in the provided code `zig.ts:152-186`
- **checkUnsafeTypeAssertion** — Validates unsafe type assertions in TypeScript `typescript.ts:420-435`
...
- **dapperCalls** — Not present in the provided code `csharp.ts:1090-1090`
- **dapperCalls** — Filters calls to only those that match DAPPER_METHODS_RE `csharp.ts:1177-1177`
- **dapperCalls** — Filters the entity's metadata calls to find Dapper method calls `csharp.ts:1293-1293`
...
- **duplicates** — Counts duplicate entities `typescript.ts:885-889`
...
- **getAntipatternHints** — Returns hints for common TypeScript antipatterns `typescript.ts:413-415`
...
- **getClosureHints** — Retrieves closure hints for an entity `typescript.ts:973-975`
...
- **hasAdd** — Determines if any call names match a set of predefined patterns for adding event listeners `typescript.ts:337-337`
- **hasAwaitCalls** — Checks for await calls in the entity `typescript.ts:282-282`
...
- **hasConfigureAwait** — Checks if the entity's metadata contains calls to ConfigureAwait `csharp.ts:915-915`
...
- **hasDbRef** — Checks if calls reference DbContext, DbSet, or _context. `csharp.ts:577-577 `csharp.ts:577-577`
...
- **hasIteration** — Checks for iteration in the entity `typescript.ts:302-302`
...
- **hasLongLivedListeners** — Checks if there are long-lived listeners `typescript.ts:982-993`
- **hasLookup** — Checks if any call names match a set of predefined patterns `typescript.ts:303-303`
- **hasMapEndpoint** — Checks if an entity has a map endpoint `csharp.ts:391-391`
- **hasMapEndpoint** — Parses calls to determine if MapPost, MapPut, or MapPatch are present `csharp.ts:413-413`
- **hasMaterializer** — Determines if the entity has calls to materializer methods like ToList `csharp.ts:573-573`
- **hasMaterializer** — Checks if the entity's metadata contains calls to materializer methods `csharp.ts:801-801`
- **hasParse** — Checks if any call names match a specific pattern for parsing JSON `typescript.ts:368-368`
- **hasPrivateTarget** — Checks if a target is private `typescript.ts:644-644`
...
- **hasReduce** — Determines if any call names match a specific pattern for reduce `typescript.ts:388-388`
...
- **hasRemove** — Determines if any call names match a set of predefined patterns for removing event listeners `typescript.ts:338-338`
- **hasSeed** — Checks if a call to random functions includes a seed `python.ts:471-471`
...
- **hasSpyOn** — Checks if a spy is on a target `typescript.ts:638-638`
- **hasStringify** — Checks if any call names match a specific pattern for stringifying JSON `typescript.ts:369-369`
...
- **isRedosVulnerable** — Checks if an entity is vulnerable to Redos `typescript.ts:1166-1178`
...
- **nullableParams** — Specifies parameters that can be null `typescript.ts:764-768`
- **optionalCount** — Counts the number of optional properties `typescript.ts:174-174`
...
- **posParams** — Filters parameters to exclude "self", "cls", and those starting with "*" `python.ts:425-425`
- **putCalls** — Not applicable in this context `java.ts:233-233`
- **queryCalls** — Not present in the provided code `csharp.ts:1323-1323`
- **randomCalls** — Filters calls to random functions `python.ts:465-468`
...
- **removeCalls** — Filters calls where the name is "remove" and the target includes "ThreadLocal", "threadLocal", or "THREAD_LOCAL" `java.ts:195-197`
- **removeCalls** — Not applicable in this context `java.ts:141-142`
- **resourceTypes** — Not applicable in this context `java.ts:339-339`
- **reverseImports** — Reverses the import statements `typescript.ts:925-947`
...
- **saveCount** — Filters calls to count SaveChanges or SaveChangesAsync calls `csharp.ts:605-605`
...
- **snapshotCount** — Counts the number of snapshots `typescript.ts:620-620`
- **splitUnionType** — Splits a union type into individual types `typescript.ts:1145-1163`
- **spyTargets** — Represents the targets of spies `typescript.ts:642-642`
- **stringParams** — Stores string parameters for an entity `typescript.ts:665-665`
- **unconstrained** — Represents an unconstrained type `typescript.ts:547-547`
...
### Type_alias
- **AntipatternHintsLike** — Represents a set of counts and strings related to common antipatterns in TypeScript `typescript.ts:402-411`
...
- **ClosureHintsLike** — Represents closure hints `typescript.ts:960-971`
...
- **addListenerCount** — Counts the number of `addListener` calls `typescript.ts:965-965`
- **allocCallCount** — Not present in the provided code `zig.ts:241-241`
- **awaits** — Represents awaits in control `csharp.ts:510-510`
- **awaits** — Counts the number of awaits `typescript.ts:278-278`
- **awaits** — Stores await statements in an entity `typescript.ts:706-706`
...
- **branches** — Represents branch structures in an entity `typescript.ts:738-738`
- **branches** — Counts the number of branches in a control flow `typescript.ts:773-773`
- **branches** — Retrieves the control flow branches from the metadata `typescript.ts:826-826`
- **branches** — Counts branches in control flow metadata `zig.ts:22-22`
- **capturedVarCount** — Counts captured variables `typescript.ts:962-962`
- **catchType** — Represents the type of exception caught `csharp.ts:491-491`
- **catchType** — Not present in the provided code `zig.ts:74-74`, `zig.ts:159-159`
- **catchType** — Not applicable in this context `zig.ts:53-53`, `zig.ts:204-204`
- **conditionalFieldAddCount** — Not applicable in this context `typescript.ts:143-143`
- **constraint** — Represents a constraint in TypeScript `typescript.ts:544-544`
...
- **defaultValue** — Represents the default value of a parameter `python.ts:30-30`
...
- **doubleAssertionCount** — Represents the count of double assertions in the entity `typescript.ts:404-404`
- **dunderMethods** — Stores the names of double-underscore methods `python.ts:565-565`
- **dynamicPropAccessCount** — Counts dynamic property access in loops `typescript.ts:218-218`
- **dynamicPropAccessCount** — Counts dynamic property accesses in an entity `typescript.ts:734-734`
- **end** — Represents the end line of an entity `typescript.ts:705-705`
- **evalInClosureCount** — Counts occurrences of `eval` within closures `typescript.ts:964-964`
- **exceptions** — Represents exceptions in control flow `csharp.ts:508-508`
- **exceptions** — Not applicable in this context `go.ts:16-16`
- **exceptions** — Detects empty catch blocks in Java `java.ts:203-203`
- **exceptions** — Empty catch block in Java `java.ts:45-45`
- **exceptions** — Retrieves exceptions metadata from control flow `python.ts:12-12`
- **exceptions** — Async void function — exceptions will crash the process `typescript.ts:31-31`
- **exceptions** — Extracts control flow metadata for exceptions `typescript.ts:74-74`
- **exceptions** — Defines an array of exception metadata, including type and catchType `zig.ts:74-74`, `zig.ts:98-98`
- **exceptions** — Represents an array of exception types and their catch types `zig.ts:53-53`, `zig.ts:159-159`
- **exceptions** — Represents a custom detector for missing defer free `zig.ts:22-22`, `zig.ts:204-204`
...
- **fullObjectCaptureCount** — Counts full object captures `typescript.ts:963-963`
...
- **innerFunctionCount** — Counts inner functions `typescript.ts:961-961`
- **innerHtmlAssignCount** — Represents the count of innerHTML assignments in the entity `typescript.ts:407-407`
- **isAwait** — Determines if the entity is an await `typescript.ts:279-279`
- **isDefault** — Indicates if an entity is a default export `typescript.ts:852-852`
...
- **isNamespace** — Indicates if an entity is a namespace `typescript.ts:852-852`
- **isOptional** — Determines if a property is optional `typescript.ts:171-171`
...
- **line** — Represents a line in the code `csharp.ts:490-490`
- **line** — Represents a line number `csharp.ts:498-498`
- **line** — Represents the line number in the entity's metadata `csharp.ts:503-503`
- **line** — Represents the line number in the entity's metadata branches `csharp.ts:506-506`
- **line** — Represents the line number in the entity's metadata returns. `csharp.ts:509-5 `csharp.ts:509-509`
- **line** — Represents an array of awaits with location details `typescript.ts:705-705`
- **line** — Represents the start line of a loop `typescript.ts:705-705`
- **line** — Represents the line number of an await `typescript.ts:706-706`
...
- **linesOfCode** — Counts the number of lines of code `typescript.ts:199-199`
- **linesOfCode** — Retrieves the lines of code metric from metadata `typescript.ts:551-551`
- **linesOfCode** — Represents the lines of code metric `typescript.ts:832-832`
- **local** — A local name within a namespace `python.ts:56-56`
- **location** — Stores the location of an await statement `typescript.ts:705-705`
- **location** — Stores the location of an entity `typescript.ts:706-706`
- **loops** — Represents loops in control flow `csharp.ts:507-507`
- **loops** — Not applicable in this context `csharp.ts:125-125`
- **loops** — Mutable static field in Java `java.ts:77-77`
- **loops** — Represents the control flow loops metadata `java.ts:106-106`
- **loops** — Detects loops in the code `python.ts:331-331`
- **loops** — Counts the number of loops `typescript.ts:198-198`
- **loops** — Represents loop structures in an entity `typescript.ts:226-226`
- **loops** — Extracts loop metadata from entity metadata `typescript.ts:298-298`
- **loops** — Represents the loops in the control flow `typescript.ts:705-705`
- **loops** — Extracts control flow metadata for loops `typescript.ts:738-738`
- **mapNewCount** — Counts the number of `new Map()` instances `typescript.ts:967-967`
...
- **name** — Extracts parameters from entity metadata `common.ts:53-53`
- **name** — Extracts the name of the entity's metadata calls `csharp.ts:191-191`
- **name** — csharp.ts:301-301` — Parses the calls metadata of the entity `csharp.ts:211-211`
- **name** — Represents the name of an entity `csharp.ts:226-226`
- **name** — Extracts the name from the metadata calls array. `csharp.ts `csharp.ts:233-233`
- **name** — Parses the metadata to extract call names `csharp.ts:256-256`, `java.ts:330-330`
- **name** — Extracts the name from the metadata calls array `csharp.ts:281-281`, `csharp.ts:1089-1089`, `csharp.ts:1176-1176`, `csharp.ts:1292-1292`, `csharp.ts:1321-1321`, `go.ts:36-36`
- **name** — Extracts the name from the metadata calls array. `csharp.ts:73 `csharp.ts:301-301`
- **name** — Not present in the provided code `csharp.ts:320-320`
- **name** — Extracts an array of call metadata from the entity's metadata `csharp.ts:390-390`
- **name** — Parses the metadata to extract call names. `csharp.ts:841-841 `csharp.ts:412-412`
- **name** — csharp.ts:1000-1000` — Parses the metadata to extract call names and targets `csharp.ts:417-417`
- **name** — Extracts an array of call names from the entity metadata `csharp.ts:442-442`
- **name** — csharp.ts:1292-1292` — Parses the metadata to extract an array of call names `csharp.ts:466-466`
- **name** — csharp.ts:1321-1321` — Parses the metadata to extract an array of call names `csharp.ts:571-571`
- **name** — Parses the metadata to extract parameter names and types `csharp.ts:604-604`
- **name** — Parses the metadata to extract decorator names `csharp.ts:686-686`
- **name** — Parses the metadata to extract call names and targets `csharp.ts:736-736`
- **name** — Extracts the name from the calls metadata `csharp.ts:775-775`
- **name** — csharp.ts:281-281` — Parses the parameters metadata of the entity `csharp.ts:169-169`, `csharp.ts:800-800`
- **name** — Extracts the name of calls from the entity metadata `csharp.ts:151-151`, `csharp.ts:841-841`
- **name** — csharp.ts:122-122` — Extracts the metadata calls as an array of call names or undefined. `csharp.ts `csharp.ts:122-122`, `csharp.ts:890-890`
- **name** — Extracts the names of decorators from an entity's metadata. `csharp.ts:191-191 `csharp.ts:103-103`, `csharp.ts:919-919`
- **name** — Not applicable in this context `csharp.ts:74-74`, `csharp.ts:1000-1000`
- **name** — The name of the function `go.ts:12-12`
- **name** — Extracts calls metadata as an array of objects with name properties `java.ts:151-151`, `java.ts:183-183`
- **name** — Extracts calls metadata as an array of objects with name and target properties `java.ts:128-128`, `java.ts:230-230`
- **name** — Represents a function to check for raw types in Java entities `java.ts:95-95`, `java.ts:279-279`
- **name** — Represents the name of a variable or entity `python.ts:420-420`
- **name** — The name of the parameter `python.ts:99-99`, `python.ts:440-440`
- **name** — The name of the function or method `python.ts:28-28`
- **name** — Extracts call names from entity metadata `typescript.ts:279-279`
- **name** — Stores the name of an entity `typescript.ts:297-297`
- **name** — Retrieves the name `typescript.ts:333-333`
- **name** — Extracts an array of call names from metadata `typescript.ts:365-365`
- **name** — Extracts an array of call names with target from metadata. `typescript.ts `typescript.ts:384-384`
- **name** — Represents the name of a call `typescript.ts:544-544`
- **name** — Represents the name of a parameter `typescript.ts:617-617`
- **name** — Parses the metadata parameters into an array of objects with name and type properties `typescript.ts:635-635`
- **name** — Stores parameters with names and types `typescript.ts:684-684`
- **name** — Stores parameters with names and optional types `typescript.ts:759-759`
- **name** — Stores calls with names and targets `typescript.ts:823-823`
- **name** — Gets the name of the entity `typescript.ts:67-67`, `typescript.ts:985-985`
- **name** — Parses the metadata to extract an array of call entities `zig.ts:91-91`, `zig.ts:165-165`
- **name** — Extracts the "calls" metadata from the entity, casting it to an array of objects with optional "name" and "target" properties `zig.ts:43-43`, `zig.ts:193-193`
- **name** — Extracts the name from the entity metadata `zig.ts:14-14`, `zig.ts:277-277`
...
- **nonNullAssertionCount** — Represents the count of non-null assertions in the entity `typescript.ts:405-405`
...
- **optional** — Indicates whether an entity is optional `typescript.ts:759-759`
- **orWithDefaultCount** — Represents the count of or-with-default expressions in the entity `typescript.ts:408-408`
- **paramMutationCount** — Represents the count of parameter mutations in the entity `typescript.ts:409-409`
- **properties** — Represents properties of the class with their setter availability `python.ts:566-566`
- **regexLiterals** — Stores an array of regex literals in the entity `typescript.ts:410-410`
- **removeListenerCount** — Counts the number of `removeListener` calls `typescript.ts:966-966`
- **reRaiseDifferentType** — Detects re-raises of exceptions of different types `python.ts:661-661`
- **returnCount** — Not applicable in this context `go.ts:58-58`
- **returnCount** — Parses the return count from the entity's metadata `go.ts:74-74`
...
- **setNewCount** — Counts the number of `new Set()` instances `typescript.ts:969-969`
- **source** — Stores the source code of an entity `typescript.ts:852-852`
- **source** — Represents the source of an import `typescript.ts:909-909`
- **source** — Retrieves the source of an import data `typescript.ts:929-929`
- **specifiers** — An array of local names within a namespace `python.ts:56-56`
- **specifiers** — Specifies the import specifiers `typescript.ts:852-852`
- **spreadInCallCount** — Counts spread in calls `typescript.ts:190-190`
- **spreadInCallCount** — Retrieves the spread in call count from the JIT hints metadata `typescript.ts:385-385`
- **start** — Stores the start line of an await statement. `typescript.ts `typescript.ts:705-705`
- **start** — Represents the start line of an entity `typescript.ts:706-706`
- **syncOverAsyncCount** — Represents a count of synchronous over asynchronous calls `csharp.ts:941-941`
- **target** — Parses the metadata to extract call targets `csharp.ts:211-211`, `java.ts:279-279`
- **target** — Extracts the target of calls from the entity metadata. `csharp.ts `csharp.ts:226-226`
- **target** — Represents a target entity for code analysis `csharp.ts:301-301`
- **target** — Represents the target of an entity `csharp.ts:442-442`
- **target** — Extracts the target from the metadata calls array `csharp.ts:466-466`, `csharp.ts:1089-1089`, `go.ts:36-36`
- **target** — Not present in the provided code `csharp.ts:571-571`
- **target** — Extracts an array of call metadata from the entity's metadata `csharp.ts:604-604`
- **target** — Extracts an array of call metadata from the entity's metadata. `csharp `csharp.ts:686-686`
- **target** — Extracts the target from the calls metadata `csharp.ts:736-736`
- **target** — Extracts an array of call names and their targets from the entity metadata `csharp.ts:169-169`, `csharp.ts:775-775`
- **target** — Not applicable in this context `csharp.ts:151-151`, `csharp.ts:1000-1000`
- **target** — The target of the function call `go.ts:12-12`
- **target** — Represents a static mutable field in Java `java.ts:128-128`, `java.ts:183-183`
- **target** — String concatenation in loop (optimization) `java.ts:95-95`, `java.ts:230-230`
- **target** — Represents the target of a test `typescript.ts:333-333`
- **target** — Represents the target of a call `typescript.ts:365-365`
- **target** — Stores calls with names and targets `typescript.ts:635-635`
- **target** — Extracts call targets from entity metadata `typescript.ts:67-67`, `typescript.ts:985-985`
- **target** — Extracts the "calls" metadata from the entity, casting it to an array of objects with optional "name" and "target" properties `zig.ts:91-91`, `zig.ts:165-165`
- **target** — Extracts the target of calls from the entity metadata `zig.ts:43-43`, `zig.ts:193-193`
- **target** — Extracts the target from the entity metadata `zig.ts:14-14`, `zig.ts:277-277`
- **throwNonErrorCount** — Represents the count of non-error throws in the entity `typescript.ts:406-406`
...
- **type** — Represents the type of an entity `csharp.ts:417-417`
- **type** — Extracts parameters from entity metadata. `csharp.ts:89 `csharp.ts:890-890`
- **type** — Not applicable in this context `csharp.ts:74-74`
- **type** — The type of the function call `go.ts:42-42`
- **type** — Raw types: generic used without type parameter `java.ts:20-20`
- **type** — Optionally represents the data type of a variable or entity `python.ts:441-441`
- **type** — Represents the type of a parameter `python.ts:29-29`
- **type** — The type of the parameter `python.ts:76-76`
- **type** — Determines the type of the entity `typescript.ts:90-90`, `typescript.ts:242-242`
- **type** — Represents the type of parameters `typescript.ts:660-660`
- **type** — Parses the metadata parameters into an array of objects with name, type, and optional properties `typescript.ts:759-759`
- **type** — Parses the metadata parameters into an array of objects with name and type properties `typescript.ts:823-823`
- **type** — Represents the type of an exception `zig.ts:159-159`
- **type** — Represents an array of exception types and their catch types `zig.ts:74-74`, `zig.ts:204-204`
- **type** — Not present in the provided code `zig.ts:53-53`
- **typeAssertionCount** — Represents the count of type assertions in the entity `typescript.ts:403-403`
...
- **weakMapNewCount** — Counts the number of `new WeakMap()` instances `typescript.ts:968-968`
- **weakSetNewCount** — Counts the number of `new WeakSet()` instances `typescript.ts:970-970`
